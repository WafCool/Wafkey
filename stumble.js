    ...
    setInterval(async function iStumble() {
        try {
            ...
            if (typeof data == "string" && data.includes("BANNED")) {
                ...
                const print = `[${color(date.getHours())}:${date.getMinutes()}] ` + [color(Id, "cyanBright"), color(Username), color(Country, "yellowBright"), color(Region, "blueBright"), color(Crowns, "cyanBright"), color(SkillRating, "greenBright")].join(" | ");
                console.log(print);
            }
        } catch (error) {}
    }, Number(interval));
